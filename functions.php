...
function tin_thumb_source($src,$w=375,$h=250,$customize=true){
	...
	if($timthumb==='on'&&$imgtype!='.gif'){
		$img = get_bloginfo('template_url').'/functions/timthumb.php?src='.$src.'&q=60&w='.$w.'&h='.$h.'&zc=1';
	}else{
...
function mytheme_get_avatar($avatar) {
	...
	if(ot_get_option('enable-gravatar')=='on'){
		$str = 'src="'.THEME_URI.'/images/image-pending.gif" data-original';
		$avatar = str_ireplace('src',$str,$avatar);
		$avatar = str_ireplace('class="avatar','class="box-hide avatar',$avatar);
		return $avatar;
	}
	return '<img src="'.THEME_URI.'/images/image-pending.gif" data-original="'.THEME_URI.'/images/gravatar.png" class="avatar box-hide" width="50" height="50" />';
}
...
function tin_get_avatar( $id , $size='40' , $type=''){
	...
	return '<img src="'.THEME_URI.'/images/image-pending.gif" data-original="'.$avatar_url.'" class="avatar box-hide" width="'.$size.'" height="'.$size.'" />';
}
...
function convert_pre_entities( $matches ) {
	return str_replace( $matches[0], '<div class="precode clearfix"><pre class="prettyprint linenums">'.htmlspecialchars( $matches[1],ENT_COMPAT,'UTF-8' ).'</pre></div', $matches[0] );
}
