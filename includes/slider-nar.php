	...
	<div id="flexslider-right">
		<span class="slider-newest">
			...
			<?php if($thelayout=='cms'){ ?>
				...
				<a href="<?php echo get_bloginfo('url').'?layout=blog'; ?>" target="_blank" title="<?php _e('更多动态','tinection'); ?>"><?php _e('更多','tinection'); ?></a>
				...
				<article class="clr">
					<a href="<?php echo $post_url; ?>" title="<?php echo $post_title; ?>" class="fancyimg">
						...
						<?php $imgsrc = tin_thumbnail(); $imgtype = substr($imgsrc,strrpos($imgsrc,'.')); if($imgtype!='.gif'){ ?>
							<img class="box-hide" src="<?php echo THEME_URI.'/images/image-pending.gif'; ?>" data-original="<?php echo tin_thumb_source($imgsrc,98,72); ?>" alt="<?php echo $post_title; ?>">
							<span></span>
						<?php } else { ?>
							<img class="box-hide" src="<?php echo THEME_URI.'/images/image-pending.gif'; ?>" data-original="<?php echo($imgsrc);?>" alt="<?php echo $post_title; ?>" class="thumb-gif">
							<span></span>			
						<?php } ?>
						</div>
					</a>
