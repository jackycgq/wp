...
class TinTabsh extends WP_Widget {

    /*  Constructor
    /* ------------------------------------ */
    function TinTabsh() {
        parent::__construct( false, 'Tin-最佳评语-管理员指定', array('description' => 'Tinection-以选项卡式显示文章、评论等内容', 'classname' => 'widget_tin_tabs') );;
    }
    ...
    public function widget($args, $instance) {
        ...
        <div class="tin-tabs-container">
            ...
            <?php if($instance['comments_enable']) { // Recent comments enabled? ?>
                ...
                <?php $args = array('status'=>'approve','post_status'=>'publish');if(isset($instance['comment_order'])&&$instance['comment_order']=='vote'){$args['orderby']='meta_value_num';$args['meta_key']='tin_comment_voteyes';};$comments = get_comments($args); ?>
...
/*  Register widget
/* ------------------------------------ */
if ( ! function_exists( 'tin_register_widget_tabs_h' ) ) {

    function tin_register_widget_tabs_h() {
        register_widget( 'TinTabsh' );
    }

}
add_action( 'widgets_init', 'tin_register_widget_tabs_h' );